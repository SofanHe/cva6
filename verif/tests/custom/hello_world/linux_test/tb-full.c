...
int main() {
  ...
  unsigned long long rgb[3] = {0x680460419fe11bff, 0xffff91ff008e0000,
                               0x00ff107eddabffb7};
  ...
  const char *filetemplate = "input/%03d.yuv";
  const char *outfiletemplate = "%03d.yuv.bmp";
  char filename[20];
  char outfilename[20];
  ...
  for (int i=0; i<=10; ++i) {
    sprintf(filename, filetemplate, i);
    sprintf(outfilename, outfiletemplate, i);
    ...
    saveRGB888AsBMP_isax(outfilename, RGB888, dst_width, dst_height);
  }
